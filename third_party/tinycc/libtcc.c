...
{
    if (s->pod) {
        /* A POD is a flat, position-independent, freestanding image linked at
           base 0 and wrapped by tccpod.c.  Pin the link settings that shape it
           regardless of how the driver arrived here: a static exe, no libc/crt,
           text at 0, and a 4 KiB RX/RW split (not the 64 KiB ELF page). */
...
static int tcc_add_binary(TCCState *s1, int flags, const char *filename, int fd)
{
    ...
    case AFF_BINTYPE_PKT:
        ret = tcc_load_packet(s1, fd);
        break;
...
LIBTCCAPI int tcc_add_library(TCCState *s, const char *libraryname)
{
...
#endif
        "%s/lib%s.a",
        "%s/%s.PKT",    /* BerryBasiC seed packet: -lMATH finds MATH.PKT */
        NULL
    };
    ...
    if (*libraryname == ':') {
        ...
        if (s->static_link)
            pp += sizeof(libs) / sizeof(*libs) - 3; /* only "%s/lib%s.a" and "%s/%s.PKT" */
        while (*pp) {
...
PUB_FUNC int tcc_parse_args(TCCState *s, int *pargc, char ***pargv)
{
    ...
    while (optind < argc) {
        ...
        case TCC_OPTION_nostdlib:
            ...
            break;
        case TCC_OPTION_pod:
            /* Emit a BerryBasiC POD.  The link-shaping settings are pinned in
