...
# ifdef TCC_POD
   /* The BerryBasiC card is an 8.3 FAT volume: file names are upper case, so
      path/extension comparisons must be case-insensitive (PROG.C is C source). */
#  define PATHCMP stricmp
# else
...
/* A POD build runs on the target but cannot JIT (-run needs mmap/signal it does
   not have), so it is deliberately treated as a cross compiler: no native -run,
   no backtrace. It still emits code for its own arch. */
#ifdef TCC_POD
# undef TCC_IS_NATIVE
...
struct TCCState {
    ...
    unsigned char *pod_rloc;
    int pod_nreloc, pod_rloc_cap;
    /* capabilities unioned in from every .PKT member pulled into the link
       (tcc_load_packet); ORed into the POD header caps by tcc_output_pod. */
    uint64_t pod_lib_caps;
...
#define AFF_BINTYPE_PKT 5   /* BerryBasiC seed packet (.PKT) */
