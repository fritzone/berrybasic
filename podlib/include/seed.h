...
struct seed_header {
    ...
    uint32_t image_size;  // total in-memory footprint incl. .bss (tcc -seed stamps
                          //   it; 0 = unknown, so the loader assumes a .bss margin)
};
...
// trampoline (below) stashes the services pointer in `berry_svc` (declared in
// berry_services.h and shared with PODs), which the library routes through.
// ---------------------------------------------------------------------------
