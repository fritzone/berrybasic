...
FILE *fopen(const char *path, const char *mode) {
    ...
    int fh = pod_svc->file_open(path, smode);
    if (!fh && append) fh = pod_svc->file_open(path, POD_OPEN_WRITE);
    if (!fh) return NULL;
    FILE *fp = malloc(sizeof(FILE));
    if (!fp) { pod_svc->file_close(fh); return NULL; }
    fp->kind = F_FILE; fp->fh = fh; fp->ungot = -1; fp->eof = 0; fp->err = 0;
    if (append) pod_svc->file_seek(fh, 0, SEEK_END);
    return fp;
}
...
FILE *fdopen(int fd, const char *mode) {
    ...
    if (!h) return NULL;
    FILE *fp = malloc(sizeof(FILE));
    if (!fp) { pod_svc->file_close(h); return NULL; }
...
int fclose(FILE *fp) {
    ...
    int r = pod_svc->file_close(fp->fh);
    free(fp);
    return r < 0 ? EOF : 0;
}
...
int fgetc(FILE *fp) {
    ...
    if (fp->kind == F_CIN) return pod_svc->getkey() & 0xFF;
    if (fp->kind != F_FILE) return EOF;
    unsigned char b;
    int n = pod_svc->file_read(fp->fh, &b, 1);
...
size_t fread(void *ptr, size_t size, size_t nmemb, FILE *fp) {
    ...
    if (fp->kind == F_CIN) { while (done < total) p[done++] = (unsigned char)(pod_svc->getkey() & 0xFF); return done / size; }
    if (fp->kind != F_FILE) return 0;
    while (done < total) {
        int n = pod_svc->file_read(fp->fh, p + done, (int)(total - done));
...
int fputc(int c, FILE *fp) {
    ...
    if (fp->kind == F_COUT) { pod_svc->putc(b); return b; }
    if (fp->kind != F_FILE) return EOF;
    int n = pod_svc->file_write(fp->fh, &b, 1);
...
size_t fwrite(const void *ptr, size_t size, size_t nmemb, FILE *fp) {
    ...
    if (fp->kind == F_COUT) { pod_svc->puts((const char *)p, (int)total); return nmemb; }
    if (fp->kind != F_FILE) return 0;
    size_t done = 0;
    while (done < total) {
        int n = pod_svc->file_write(fp->fh, p + done, (int)(total - done));
        if (n <= 0) { fp->err = 1; break; }
        done += (size_t)n;
    }
    return done / size;
...
int fseek(FILE *fp, long off, int whence) {
    ...
    fp->ungot = -1; fp->eof = 0;
    long r = pod_svc->file_seek(fp->fh, off, whence);
    return r < 0 ? -1 : 0;
}
long ftell(FILE *fp) {
    if (!fp || fp->kind != F_FILE) return -1;
    long r = pod_svc->file_seek(fp->fh, 0, SEEK_CUR);
    if (r < 0) return -1;
    if (fp->ungot >= 0 && r > 0) r--;
    return r;
}
...
int  remove(const char *path) { return pod_svc->file_remove(path) < 0 ? -1 : 0; }
